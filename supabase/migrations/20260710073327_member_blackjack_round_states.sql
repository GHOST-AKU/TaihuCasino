create table if not exists public.member_blackjack_round_states (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  table_session_id uuid not null references public.member_table_sessions(id) on delete cascade,
  game_slug text not null default 'blackjack' check (game_slug = 'blackjack'),
  status text not null default 'active' check (status in ('active', 'settled', 'voided')),
  phase text not null check (phase in ('insurance', 'player_turn', 'dealer_turn', 'settled', 'voided')),
  version integer not null default 1 check (version >= 1),
  deck jsonb not null,
  dealer_cards jsonb not null,
  player_hands jsonb not null,
  current_hand_index integer not null default 0 check (current_hand_index >= 0),
  stake numeric(14, 2) not null check (stake > 0),
  chip_balance_before numeric(14, 2) not null check (chip_balance_before >= 0),
  chip_balance_after numeric(14, 2) not null check (chip_balance_after >= 0),
  insurance_bet numeric(14, 2) not null default 0 check (insurance_bet >= 0),
  insurance_offered boolean not null default false,
  insurance_resolved boolean not null default true,
  ...
  final_round_id uuid references public.member_game_rounds(id),
  expires_at timestamptz not null,
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
  check (status <> 'settled' or final_round_id is not null),
  check (status <> 'voided' or final_round_id is null)
);

create unique index if not exists member_blackjack_round_states_user_idempotency_key_idx
on public.member_blackjack_round_states (user_id, idempotency_key)
where idempotency_key is not null;

create unique index if not exists member_blackjack_round_states_one_active_session_idx
on public.member_blackjack_round_states (user_id, table_session_id)
where status = 'active';

create index if not exists member_blackjack_round_states_user_status_expires_idx
on public.member_blackjack_round_states (user_id, status, expires_at);

create index if not exists member_blackjack_round_states_final_round_idx
on public.member_blackjack_round_states (final_round_id)
where final_round_id is not null;

alter table public.member_blackjack_round_states enable row level security;

comment on table public.member_blackjack_round_states is
  'Server-only blackjack active round state. Contains hidden deck and command idempotency data; do not grant direct authenticated access.';

revoke all on table public.member_blackjack_round_states from public;
revoke all on table public.member_blackjack_round_states from anon;
revoke all on table public.member_blackjack_round_states from authenticated;
grant select, insert, update, delete on table public.member_blackjack_round_states to service_role;
...
revoke execute on function public.settle_member_table_session_round(uuid, uuid, text, text, numeric, numeric, text, jsonb, jsonb, text, uuid) from public;
revoke execute on function public.settle_member_table_session_round(uuid, uuid, text, text, numeric, numeric, text, jsonb, jsonb, text, uuid) from anon;
revoke execute on function public.settle_member_table_session_round(uuid, uuid, text, text, numeric, numeric, text, jsonb, jsonb, text, uuid) from authenticated;
grant execute on function public.settle_member_table_session_round(uuid, uuid, text, text, numeric, numeric, text, jsonb, jsonb, text, uuid) to service_role;
