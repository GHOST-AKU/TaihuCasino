...
- Configure OAuth providers used by the login page: Google, Apple, Microsoft/Azure, Facebook, X, Discord, and Twitch. Amazon is explicitly out of scope. / 配置登录页使用的 OAuth provider：Google、Apple、Microsoft/Azure、Facebook、X、Discord 和 Twitch。Amazon 已明确排除在支持范围之外。
...
- The proxy issues a host-only, HttpOnly UUIDv4 `member-device-id` cookie. Shared API limits consume independent client/IP, device, session, user, and normalized identifier buckets; the device cookie is a soft abuse signal and never replaces IP/account limits. / 代理会签发仅限当前主机、HttpOnly 的 UUIDv4 `member-device-id` cookie。共享 API 限流会分别消耗客户端/IP、设备、session、用户及规范化标识桶；设备 cookie 只是软性滥用信号，不能替代 IP 或账户限流。
